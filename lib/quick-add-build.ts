...
// ---------------------------------------------------------------------------
// Time + allergen helpers (exported for tests / streaming preview)
// ---------------------------------------------------------------------------
...
export function mapAllergenHints(hints: string[] | undefined): {
  codes: AllergenCode[]
  unmapped: string
} {
...
function clampWithActivitySchema(data: {
  dayId: string
  title?: string | undefined
  description?: string | undefined
  startTime?: string | undefined
  endTime?: string | undefined
  expectedCovers?: number | undefined
  notes?: string | undefined
  allergens?: AllergenCode[] | undefined
}): {
  title: string
  description: string
  startTime: string
  ...
  const p = activitySchema.partial().extend({ dayId: z.string().uuid() }).safeParse(row)
  const base = (() => {
    if (!p.success) {
      return {
    ...
    return {
      ...
      endTime: o.endTime
        ...
        : data.endTime
          ...
          : '',
      expectedCovers:
        o.expectedCovers != null
          ? String(o.expectedCovers)
          : data.expectedCovers != null
            ? String(data.expectedCovers)
            : '',
...
function clampReservation(o: { dayId: string } & Partial<ReservationFormData>): {
  guestName: string
  guestCount: string
  startTime: string
  endTime: string
  ...
  const parsed = reservationSchema.partial().extend({ dayId: z.string().uuid() }).safeParse(o)
  if (!parsed.success) {
    return {
    ...
    guestCount:
      d.guestCount != null
        ...
        : o.guestCount != null
          ...
          : '',
    startTime:
      d.startTime != null && String(d.startTime) !== ''
        ? normalizeToTimeInput(String(d.startTime))
        : o.startTime
          ? normalizeToTimeInput(String(o.startTime))
          : '',
    endTime:
      d.endTime != null && String(d.endTime) !== ''
        ? normalizeToTimeInput(String(d.endTime))
        : o.endTime
          ? normalizeToTimeInput(String(o.endTime))
          : '',
    notes: (d.notes as string) ?? o.notes ?? '',
    tableBreakdown: Array.isArray(d.tableBreakdown)
      ? d.tableBreakdown
      : Array.isArray(o.tableBreakdown)
        ? o.tableBreakdown
        : [],
  }
}

function clampBreakfast(o: { dayId: string } & Partial<CreateBreakfastFormData>): {
  groupName: string
  guestCount: string
  startTime: string
  notes: string
  ...
  const parsed = createBreakfastSchema.partial().extend({ dayId: z.string().uuid() }).safeParse(o)
  if (!parsed.success) {
    return {
...
export function buildDataFromLlm(
  ...
  if (out.kind === 'activity') {
    ...
    return {
      ...
      modelFilledFieldKeys: modelKeysFromLlmFields(
        out.fields as unknown as Record<string, unknown>
      ),
    }
  }
  ...
  if (out.kind === 'breakfast') {
    ...
    const merged: { dayId: string } & Partial<CreateBreakfastFormData> = {
      ...
      startTime: strip ? undefined : f.startTime ? normalizeToTimeInput(f.startTime) : undefined,
      notes: n.notes,
