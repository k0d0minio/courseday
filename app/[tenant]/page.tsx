...
export default async function TenantHomePage({
  ...
  const [{ role }, tenantData] = await Promise.all([
    requireTenantMember(),
    supabase.from('tenants').select('timezone, onboarding_completed').eq('id', tenant.id).single(),
  ])
