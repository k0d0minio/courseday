...
> to go for a given task. Keep the top short; the operational detail below it is the
> repo's own, accumulated over the build and worth keeping.
>
...
**courseday** — *"Run your golf club's day in one view."* A multi-tenant daily-operations
platform for golf clubs: reception, restaurant, pro shop and floor staff all work from one
shared page — the day's programme, reservations, breakfasts, covers and staff shifts,
visible to every role.

Each club is a **tenant** with its own subdomain, team, branding and language. Next.js 15
App Router + React 19, Supabase (Postgres, Auth, RLS) as the source of truth, Upstash Redis
for tenant routing, `next-intl` in **four languages** (en/fr/de/es), Tailwind v4 +
shadcn/ui, Sentry, Vitest + Playwright. Deployed on Vercel at **courseday.golf** (wildcard
`*.courseday.golf` serves the tenant subdomains).

> **This repo is dormant** — `.icm/dormant`. It was built out hard across April–May 2026
> (~517 commits) and has been parked since; the production deployment stands and the
> domain is live, but no work is planned. Dormancy silences the off-ticket hygiene check
> and nothing else — every other estate check still applies here.
>
> **Waking it:** `git rm .icm/dormant` in the same commit as the first stub. A repo is
> dormant, never a ticket — a dormant repo that gains a stub drops the marker with it.

**Register:** [`.icm/project.md`](.icm/project.md) is what this project is *for* and the
decisions behind it. This file is *where to go*; that one is *what and why*. Neither
restates the other.
...
| The task | Go to |
|---|---|
| Tenant app — the club-facing product | [`app/[tenant]/`](app/) — `day/`, `schedule/`, `my-schedule/`, `notifications/`, `profile/`, `admin/`, `auth/`, `pwa/` |
| Platform surfaces — landing, sign-up, demo | [`app/(platform)/`](app/) — `new/`, `auth/`, `book-a-demo/` |
| Superadmin dashboard, feature flags | [`app/admin/`](app/admin/) — flags defined in [`lib/feature-flags.ts`](lib/feature-flags.ts) |
| Any mutation or data read | [`app/actions/`](app/actions/) — the server-action inventory is below |
| HTTP endpoints, cron, AI | [`app/api/`](app/api/) — `cron/`, `daily-brief/`, `quick-add/`, `geocode/`, `mutations/`, `calendar/`, `demo/` |
| Subdomain resolution, tenant rewrite | [`middleware.ts`](middleware.ts) — plus Redis cache in [`lib/`](lib/) |
| Schema — tables, RLS, anything DB | [`supabase/migrations/`](supabase/migrations/) — 55 migrations; read the conventions below before adding one |
| Copy in any language | [`messages/`](messages/) — `en` · `fr` · `de` · `es`; **`grep` the key, never full-read** |
| Components, design system | [`components/`](components/) — `ui/` holds the shadcn primitives and the button contract |
| Tests | [`tests/`](tests/) — Vitest, mirroring source paths; `tests/e2e/` is Playwright |
| CI | [`.github/workflows/`](.github/workflows/) — `ci.yml`, `migration-check.yml` |
...
- **CI is the source of truth.** Never run `build`/`lint`/`typecheck`/`test`/`format`/`dev`
  locally — push and read the checks. This is the estate rule and it is also this repo's
  own hard-won one: the full ban list, and what to do when CI goes red, is in
  *Agent Workflow Rules* below. `opencode.jsonc` and `.claude/settings.json` enforce it.
- **Planning is tickets.** Any plan or backlog becomes stubs in `.icm/intake/`, never a
  ...
  on a `claude/` branch.
- **Migrations are files, never MCP calls.** Write SQL into `supabase/migrations/`; CI
  deploys it. Check the highest existing number first — parallel agents collide otherwise.
- **All four languages move together.** A string added to `en.json` is added to `fr`, `de`
  and `es` in the same change. A missing key is a visible defect, not a fallback.
...
*Everything below is courseday's own operating detail, written during the build. It is the
reason this file is longer than a routing table.*

## Personality

Caveman mode is always enabled in this project. Follow all rules from the `caveman-mode`
skill for every response. The skill library lives in `.claude/skills/` (mirrored in
`.agents/skills/`, pinned by `skills-lock.json`).
...
Work is planned as **epics and stubs in [`.icm/intake/`](.icm/intake/)** — the estate
ticket contract, summarised in that folder's README. There is no `TODO.md` and no
backlog file at the repo root.

> A root `TICKETS.md` used to hold the backlog; it was deleted in `422cf40`
> (2026-04-05) and this section pointed at it until adoption. If you find a reference to
> `TICKETS.md` anywhere else in this repo, it is stale — `.icm/intake/` is the backlog.

**Supabase migrations** keep the rule that section carried: implement SQL in
`supabase/migrations/` and let CI deploy it — never apply migrations through the Supabase
MCP tools, and never defer migration work to the user.
