...
vi.mock('@/app/actions/days', () => ({
  ensureDayExists: vi
    .fn()
    .mockResolvedValue({ success: true, data: { id: 'day-1', date_iso: '2024-06-01' } }),
}))
vi.mock('@/lib/rate-limit', () => ({
  mutationRateLimit: vi.fn().mockResolvedValue({ success: true }),
  authRateLimit: vi.fn().mockResolvedValue({ success: true }),
  dailyBriefRateLimit: vi.fn().mockResolvedValue({ success: true }),
  quickAddRateLimit: vi.fn().mockResolvedValue({ success: true }),
}))
