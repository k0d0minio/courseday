...
test.describe('Settings', () => {
  ...
  test('branding palette persists and previews live', async ({ signedInPage, tenantUrl }) => {
    ...
    const selectedBefore = await signedInPage
    ...
    const primaryBefore = await signedInPage.evaluate(() =>
      getComputedStyle(document.querySelector('.tenant-themed') as HTMLElement)
        .getPropertyValue('--tenant-light-primary')
        .trim()
    )
    ...
    await expect(signedInPage.getByRole('radio', { name: targetPalette })).toHaveAttribute(
      'aria-checked',
      'true'
    )
  ...
  test('branding city search shows mocked results', async ({ signedInPage, tenantUrl }) => {
    await signedInPage.route('**/api/geocode**', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify([{ name: 'Paris', lat: 48.8566, lon: 2.3522, country: 'FR' }]),
      })
    })
    ...
    await expect(signedInPage.getByRole('option', { name: /Paris/i })).toBeVisible({
      timeout: 8000,
    })
  })
