...
import { saveTemplate, updateTemplate } from '@/app/actions/schedule-templates'
...
export function ScheduleTemplateEditorDialog({ open, onOpenChange, template, onSaved }: Props) {
  ...
  const resetFromProps = useCallback(() => {
    if (template) {
      ...
      setLines(template.items.length > 0 ? template.items.map((i) => ({ ...i })) : [emptyLine()])
    } else {
      setName('')
      setLines([emptyLine()])
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn('max-h-[min(90vh,880px)] overflow-y-auto sm:max-w-2xl', 'gap-0 p-0')}
      >
        <DialogHeader className="p-6 pb-2">
          <DialogTitle>{template ? t('editorEditTitle') : t('editorCreateTitle')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 px-6 pb-2">
          ...
          <div className="space-y-3">
            <div className="flex items-baseline justify-between gap-2">
              ...
              {metaLoading && (
                <span className="text-muted-foreground text-xs">{t('loadingMeta')}</span>
              )}
            </div>
              ...
              <div key={index} className="bg-muted/30 space-y-3 rounded-lg border p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0 flex-1 space-y-1.5">
                    <Label htmlFor={`st-title-${index}`}>{tAct('titleLabel')}</Label>
                    <Input
                      id={`st-title-${index}`}
                      value={line.title}
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => setLine(index, { end_time: timeFromInput(e.target.value) })}
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor={`st-cov-${index}`}>{tAct('expectedCoversLabel')}</Label>
                    <Input
                      id={`st-cov-${index}`}
                      type="number"
                      min={0}
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setLine(index, { poc_id: v === NONE ? null : v })}
                    >
                      <SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    value={line.notes ?? ''}
                    onChange={(e) => setLine(index, { notes: e.target.value || null })}
                  />
                </div>
              </div>
            ))}
