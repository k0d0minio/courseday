    ...
    <SectionShell tone="soft">
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-3">
        {items.map((it) => (
          <article
            key={it.title}
            className="bg-background/70 rounded-xl border border-black/5 p-6 backdrop-blur-sm dark:border-white/5"
          >
            <h3 className="font-display text-xl leading-snug font-medium">{it.title}</h3>
            <p className="text-muted-foreground mt-2 text-sm leading-relaxed">{it.body}</p>
          </article>
        ))}
      </div>
    </SectionShell>
